...
Three deterministic moving magnetic moments with analytical field superposition and bounded midpoint streamline integration.
...
The pure model is in `source/MagneticDipolesModel.h`; its public state and deterministic signature support focused non-graphics validation. The SDK adapter uses one batched mesh, a transparent background, and balanced view, matrix and style scopes. Field units are normalized; Stroke Width is in pixels, except the crystal's Cell Coverage, which is a cell fraction.
