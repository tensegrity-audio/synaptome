...
CONTRACT_ENTRIES: tuple[ContractEntry, ...] = (
    ContractEntry(
        name="Radio TLV envelope/types",
        status="validated",
        sources=(Path("common/radio_schema.h"),),
        validator_command="python tools\\osc_regression.py",
        fixtures=(Path("tools/testdata/osc_regression/phase1_radio_capture.json"),),
        notes="Schema source is v4; current OSC regression fixture replays system, control, sensor, parameter, and UI catalog packets.",
    ),
    ContractEntry(
        name="Radio generated device config",
        status="validated",
        sources=(Path("synaptome/bin/data/config/netmap.json"), Path("tools/gen_radio_config.py"), Path("common/radio_config.h")),
        validator_command="python tools\\gen_radio_config.py --check",
        notes="Read-only generated-header drift check is available.",
    ),
    ContractEntry(
        name="Netmap/radio config source",
        status="validated",
        sources=(Path("synaptome/bin/data/config/netmap.json"),),
        validator_command="python tools\\validate_configs.py synaptome/bin/data/config/netmap.json",
        notes="Ownership remains unresolved.",
    ),
    ContractEntry(
        name="Gateway OSC output catalog",
        status="validated",
        sources=(Path("tensegrity_gateway/src/main.cpp"), Path("docs/osc_catalog.md")),
        validator_command="python tools\\osc_regression.py",
        fixtures=(Path("tools/testdata/osc_regression/phase1_expected_osc.json"),),
        notes="Fixtures cover system/control/sensor/param OSC and current UI catalog no-OSC gateway behavior.",
    ),
    ContractEntry(
        name="Radio UI/catalog exchange",
        status="validated",
        sources=(
            Path("common/radio_schema.h"),
            Path("common/radio_config.h"),
            Path("tensegrity_matrixportal/src/main.cpp"),
            Path("tensegrity_cyberdeck/src/main.cpp"),
            Path("tensegrity_cyberdeck/src/ui/catalog_manager.cpp"),
            Path("tools/radio_ui_catalog_regression.py"),
        ),
        validator_command="python tools\\radio_ui_catalog_regression.py",
        fixtures=(
            Path("tools/testdata/osc_regression/phase1_radio_capture.json"),
            Path("tools/testdata/radio_ui_catalog/phase1_radio_capture.json"),
            Path("tools/testdata/radio_ui_catalog/phase1_expected_controller.json"),
        ),
        notes="Gateway no-OSC guard exists; controller catalog chunk assembly and command-ack log behavior are fixture-backed.",
    ),
    ContractEntry(
        name="App OSC map",
        status="validated",
    ...
    ContractEntry(
        name="Tiny-screen UI layout primitives",
        status="validated",
        sources=(
            Path("docs/memento/ux/ui_system_guide.md"),
            Path("tensegrity_memento/tools/memento_ui_sim.py"),
            Path("tensegrity_memento/tools/memento_ui_regression.py"),
            Path("tools/validate_tiny_screen_ui_contract.py"),
            Path("tensegrity_cyberdeck/src/ui"),
            Path("tensegrity_cardputer/src/ui_renderer.cpp"),
        ),
        validator_command="python tools\\validate_tiny_screen_ui_contract.py",
        fixtures=(Path("tensegrity_memento/tests/artifacts/memento_ui"),),
        notes="Static artifact validator covers committed simulator manifests and parity artifacts; optional pixel diff still requires Pillow and hardware/firmware parity remains package follow-up.",
        check_command=(sys.executable, "tools/validate_tiny_screen_ui_contract.py"),
    ),
    ContractEntry(
        name="Agent governance/roadmap scope model",
        status="validated",
        sources=(
            Path("docs/governance/ROADMAP_TOPOLOGY.md"),
            Path("docs/docops.md"),
            Path("docs/roadmap/roadmap.md"),
            Path("docs/roadmap/in_progress/_REQUEST_TEMPLATE.md"),
            Path("tools/validate_governance_contract.py"),
        ),
        validator_command="python tools\\validate_governance_contract.py",
        fixtures=(Path("docs/roadmap/in_progress/_REQUEST_TEMPLATE.md"),),
        notes="Static lint covers scope labels, the request template, and State Summary shape for entries that opt into it; it is not roadmap correctness proof.",
        check_command=(sys.executable, "tools/validate_governance_contract.py"),
    ),
    ContractEntry(
        name="Legacy firmware payload structs",
        status="validated",
        sources=(
            Path("tensegrity_cyberdeck/include/payload.h"),
            Path("tensegrity_matrixportal/include/payload.h"),
            Path("tensegrity_gateway/src/main.cpp"),
            Path("tools/validate_legacy_payload_quarantine.py"),
        ),
        validator_command="python tools\\validate_legacy_payload_quarantine.py",
        fixtures=(Path("tensegrity_cyberdeck/include/payload.h"), Path("tensegrity_matrixportal/include/payload.h")),
        notes="Quarantine check records known historical payload coupling; it does not make legacy payloads a supported radio contract or unblock firmware extraction.",
        check_command=(sys.executable, "tools/validate_legacy_payload_quarantine.py"),
    ),
)
