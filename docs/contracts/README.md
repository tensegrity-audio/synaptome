...
| Layer package activation | Read-only package inspection plus explicit source-package runtime promotion and operator-owned preset/mapping transactions. | `artist-sdk`, `app-runtime` | Runtime owner, app contract owner | Package v1, controlled registration set, generated Runtime/build records, `LayerLibrary::loadOptInPackages`, `PackageControlTransactions` | Browser, layer catalog, Console scene load, package authors | Schema version 1; compile-time registration is explicit, runtime catalog activation defaults false, and package presets/mappings mutate live state only through explicit recoverable transactions. | `python tools\generate_element_package_registrations.py --check`; `python tools\run_element_confidence.py --package docs\examples\layer_packages\signal_bloom\layer.package.json --tier contract` | Signal Bloom package, generated records, Browser inspection, BrowserFlow package transactions, confidence and package benches | Preserve stable identities; validate before generation; preview without mutation; require explicit conflict approval; restore prior working state on failed writes. | Current; Validated | Partial: runtime discovery remains SEAC-10. |
