...
| Signal Control receive and OSC input selection | Synaptome Browser receive-mode config plus Signal Control host-audio scalar/waveform routes into Router UDP. | `app-contract`, `adapter-contract` | App contract owner | `synaptome/bin/data/config/osc-input.json`, `docs/contracts/signal_control_integration.md` | Signal Control, Synaptome Browser, `AudioAnalysisBus`, audio-reactive layers | Schema version 1. Keep `directSerial` as the default fallback and `routerUdp` as the stream-router mode. `/sensor/host/<source>/waveform` allows 64, 128, or 256 float samples. | `python tools\validate_signal_control_receive_contract.py --check` | `docs/examples/osc_input_example.json`, `tools/testdata/runtime_state/config/osc-input.json`, `tools/testdata/signal_control/expected_receive_contract.json` | Route or freshness changes must update the fixture, schema/docs, and Tensegrity producer docs together. | Current; Validated | No for the current host-audio receive path; future non-numeric gateway OSC surfacing is a separate follow-up. |
...
| Circuit Lenia presentation | Shared Lenia simulation with a fixed catalog-selected hard-isocontour circuit view. | `app-contract`, `artist-sdk` | Runtime owner, app contract owner | `LeniaLayer`, `lenia.json`, `circuit_lenia.json`, `midi-map.json` | Scenes, mappings, Browser, layer authors | Organic Lenia retains its presentation; Circuit Lenia owns a stable ID/prefix, complete circuit defaults, and seven ordinary editable OSC routes. | `python tools\validate_circuit_lenia.py` | Organic and Circuit Lenia catalogs plus BrowserFlow lifecycle and mapping-snapshot coverage | Share simulation state and lifecycle; keep circuit-only controls out of the organic UI and all OSC addresses out of the renderer. | Current; Validated | Partial: live visual acceptance and default tuning remain. |
| Artist SDK example fixture | Minimal public source/catalog/scene bundle for an artist-authored layer. | `artist-sdk` | App contract owner | `docs/examples/artist_sdk/SignalBloomLayer.*`, `docs/examples/artist_sdk/register_signal_bloom.cpp`, `docs/examples/artist_sdk/signal_bloom.layer.json`, `docs/examples/artist_sdk/signal_bloom.scene.json` | Layer authoring docs, Browser catalog, Console scenes, MIDI/OSC/sensor mapping examples, public SDK packaging | Fixture schema v1; source registration remains an explicit `Visual` descriptor plus creator until generated registration exists. | `python tools\validate_artist_sdk_example.py --check` | `docs/examples/artist_sdk/**`, `tools/testdata/artist_sdk/expected_artist_sdk_example.json` | Type ID, kind, ordered action declarations, parameter suffixes, catalog ID, scene target, and registration changes must update the SDK snapshot and docs together. | Current; Validated | Partial: package layout and non-source-edit extension mechanism remain pending. |
| Layer package activation | Read-only package inspection plus explicit source-registered runtime promotion. | `artist-sdk`, `app-runtime` | Runtime owner, app contract owner | `synaptome/bin/data/config/layer-packages.json`, `synaptome/bin/data/layers-optional/*.json`, `LayerLibrary::loadOptInPackages`, `ControlMappingHubState` inspection rows | Browser, layer catalog, Console scene load, package authors | Schema version 1; global and per-package enable flags default false; package mappings never auto-apply. | `python tools\synaptome_layer.py check docs\examples\layer_packages\signal_bloom\layer.package.json`; `python tools\validate_configs.py synaptome\bin\data\config\layer-packages.json synaptome\bin\data\layers-optional\examples.signal_bloom.json` | Signal Bloom package, runtime inspection payload, `BrowserFlowTest`, `LayerPackageBench` | Merge defaults, preset, activation overrides, then scene values; preserve asset/type/parameter IDs across revisions. | Current; Validated | Partial: source registration is still compiled into the app; generated registration or plugin loading remains later. |
...
| Scene persistence schema | Saved scene files and scene-last runtime state, including explicit mapping-snapshot ownership. | `app-runtime` | Runtime owner, app contract owner | `ofApp::encodeSceneJson`, `ofApp::loadScene`, committed scene fixtures | Runtime, console/layers, mappings, HUD/window state | Scene schema version is implicit; scene v2 contract is pending. Each omitted mapping subsection preserves its live/global state; a present router or slot-assignment snapshot is authoritative, including an explicitly empty snapshot. New saves also retain the active mapping bank. | `python tools\validate_scene_persistence_contract.py --check` | `tools/testdata/scene_persistence/expected_scene_contract.json`, `tools/testdata/runtime_state/layers/scenes/*.json`, `tools/testdata/runtime_state/config/scene-last.json` | Mapping ownership changes require a compatibility fixture and transaction-gate update; use `--live` for local app-written smoke state. | Current; Validated | Partial: live-window/show-machine fixture coverage remains follow-up, not a current extraction blocker. |
| Scene and mapping recovery | Transactional scene publication, recoverable scene/assignment writes, and non-destructive MIDI/OSC mapping import. | `app-runtime`, `adapter-contract` | Runtime owner, app contract owner | `ofApp::buildSceneApplyPlan`, `ofApp::publishScenePlan`, `writeJsonRecoverably`, `MidiRouter::load`, `MidiRouter::importMappingSnapshot` | Scene runtime, Browser, HUD, MIDI/OSC routing, controller slot assignments | A malformed or missing mapping file must leave working mappings intact. Scene and assignment writes verify temporary JSON and retain a last-known-good backup until the new file is committed. | `python tools\validate_scene_display_transaction.py` | Legacy no-mapping scene and explicit mapping-scene fixtures under `tools/testdata/runtime_state/**`; native mapping round-trip scenario in `BrowserFlowTest` | Any change to mapping precedence, rollback, or write promotion must update the source gate and native recovery scenario together. | Current; Validated | Partial: physical dual-window save/reload/restart rehearsal remains show-machine work. |
