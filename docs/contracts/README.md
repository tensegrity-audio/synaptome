...
| Layer package activation | Read-only package inspection plus explicit source-package runtime promotion. | `artist-sdk`, `app-runtime` | Runtime owner, app contract owner | Package v1, controlled registration set, generated Runtime/build records, `LayerLibrary::loadOptInPackages` | Browser, layer catalog, Console scene load, package authors | Schema version 1; compile-time registration is explicit, runtime catalog activation defaults false, and package mappings never auto-apply. | `python tools\generate_element_package_registrations.py --check`; `python tools\run_element_confidence.py --package docs\examples\layer_packages\signal_bloom\layer.package.json --tier contract` | Signal Bloom package, generated records, Browser inspection, confidence and package benches | Preserve stable identities; validate the complete set before generation and exact package/Runtime parity before creator invocation. | Current; Validated | Partial: transactional mappings and runtime discovery remain SEAC-9 and SEAC-10. |
