...
| CG-09 | External device/display contract map | Partial Validator Added | `app-contract` | MIDI, OSC, audio, webcam, media, display, hotkey, and helper input boundaries exist, but they are not yet schemas/fixtures with one consistent ownership model. Signal Control Router UDP receive and host-audio waveform routes now have a fixture-backed validator. | Promote the remaining boundaries from `docs/architecture/synaptome_external_contracts.md` into schemas/fixtures. Keep synaptome_mesh gateway ingest/fanout in Tensegrity Signal Control, with Synaptome follow-up only for payload types the app must actively consume. | Public Synaptome packaging and helper seams. |
