...
| Generated STL/model layer fixture | `docs/examples/generated_layers/stl_models/**`, `tools/testdata/generated_layers/expected_generated_layer_catalog.json` | `python tools\generated_layer_catalog_regression.py --check` | Draft generated-layer template and sidecar fixture proving one STL file expands into stable catalog-style metadata, standard model parameters, static and dynamic option metadata, and no legacy asset ID conflicts. | Draft |
| Layer Browser inspection payload | `tools/testdata/layer_browser_inspection/expected_layer_browser_inspection_payload.json`, `docs/schemas/layer_browser_inspection_payload.schema.json` | `python tools\layer_browser_inspection_payload.py --check`, `python tools\validate_configs.py tools\testdata\layer_browser_inspection\expected_layer_browser_inspection_payload.json` | Draft read-only pre-UI payload proving package and generated-layer metadata can be combined for future Browser inspection without runtime loading, layer instantiation, scene mutation, or canonical manifest changes. | Draft |
