...
| Applied parameter/action routes | Scene when explicitly embedded; otherwise operator mapping store | Visible owner selected at save/apply time | Versioned router snapshot with stable logical targets | Mapping-bank v1 target; current snapshots are legacy unversioned | Omitted route snapshot preserves live/operator routes; present empty snapshot clears them | Operator |
