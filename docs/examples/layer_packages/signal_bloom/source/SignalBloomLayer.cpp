...
void SignalBloomLayer::configure(const ofJson& config) {
    ...
    bpmMultiplier_ =
        defaults.value("bpmMultiplier", bpmMultiplier_);
    ...
    lineOpacity_ =
        normalizedDefault(defaults, "lineOpacity", lineOpacity_);
    xInput_ = normalizedDefault(defaults, "xInput", xInput_);
    yInput_ = normalizedDefault(defaults, "yInput", yInput_);
    speedInput_ =
        normalizedDefault(defaults, "speedInput", speedInput_);
...
void SignalBloomLayer::update(const LayerUpdateParams& params) {
    ...
    const float transportRate = bpmSync_
        ? std::max(0.01f, params.bpm / 120.0f) * bpmMultiplier_
        : speed_;
    const float modulation =
        1.0f + ofClamp(speedInput_, 0.0f, 1.0f) * gain_ * 2.0f;
    phase_ +=
        params.dt * transportRate * modulation *
    ...
    for (std::size_t i = 0; i < points_.size(); ++i) {
        const float t =
            static_cast<float>(i) /
            static_cast<float>(
                std::max<std::size_t>(1, points_.size() - 1));
        const float angle = t * kTwoPi * 6.0f + phase_ + twist;
        const float pulse =
            std::sin(phase_ * 2.0f + t * kTwoPi);
        const float radius =
            (0.18f + t * 0.72f) * scale_ *
            (1.0f + pulse * 0.12f * gain_);
        const float xDrift = (xInput_ - 0.5f) * 0.35f;
        const float yDrift = (yInput_ - 0.5f) * 0.35f;
        points_[i] = {
        ...
        for (ofIndexType vertex = 0; vertex < 8; ++vertex) {
            ...
            dotMesh_.setVertex(
                ...
                glm::vec3(
                    points_[i].x +
                        std::cos(vertexAngle) * dotRadius,
                    points_[i].y +
                        std::sin(vertexAngle) * dotRadius,
                    0.0f));
        }
    }
}
