...
`ElementTypeContract`, its binding mode, and one creator. Contract lookup and
...
SEAC-4 implements the static declaration authority. Public pointer-free DTOs
cover parameter kind/value, range/step, group, static options, option-source
references, quick-access order, aliases, and deprecation. All 23 built-in types
declare 786 parameters without construction. Signal Bloom is the explicit
bind-only reference; existing implementations use an exact-parity storage
adapter while their setup metadata is discarded.
...
The runtime contract, compiled declaration payload, Browser inspection,
machine-readable catalog, compatibility manifest, and human reference are
generated from one reviewed snapshot. Exact live validation covers all 23
types and 55 catalog assets. Package serialization and scene/preset/mapping
ownership and migration remain later phases.
