...
| Layers | Real SDK, explicit bind-only storage for every shipping type, controlled packages/discovery, fixture-backed catalog, and public authoring guide. | Keep the supported authoring and validation path stable. | Remove inert shared-source scratch registration statements without reopening the architecture gate. |
